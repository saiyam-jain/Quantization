...
class ErrorInjection:
    def __init__(self, layer_name='first_conv', percent=20):
        ...
        self.model = tf.keras.models.load_model('binary_model_MNIST.h5')
        (_, _), (self.test_images, self.test_labels) = tf.keras.datasets.mnist.load_data()
        self.test_images = self.test_images.reshape((10000, 28, 28, 1))

        _, self.test_acc = self.model.evaluate(self.test_images, self.test_labels)

        print(f"Test accuracy before error injection {self.test_acc * 100:.2f} %")
        ...
        print(f"Test accuracy after 2% error injection {self.test_acc * 100:.2f} %")
